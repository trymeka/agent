  ...
  return {
    async start(sessionId: string) {
      ...
      const instance = await scrapybaraClient.startBrowser({
        timeoutHours: 1,
        resolution: [options.screenSize.width, options.screenSize.height],
      });
    ...
    async takeScreenshot(sessionId: string) {
      ...
      const screenshot = await result.instance.screenshot();
      return screenshot.base64Image;
    },

    uploadScreenshot(uploadOptions: {
      screenshotBase64: string;
      sessionId: string;
      step: number;
    }) {
      return options.uploadScreenshot(uploadOptions);
    },
